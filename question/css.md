...
21. 说说flexbox（弹性盒布局模型）,以及适用场景？
	- A：
		...
		- flex成为弹性布局，可以简便、完整、响应式地实现各种页面布局。开启了弹性布局的元素称为弹性容器，其内部所有直接子元素称为弹性元素。容器中存在两条轴，主轴和交叉轴，呈90度关系。
		...
		- 弹性容器的属性有：
		  ...
		  > 4. justify-content：决定了弹性元素在主轴的对其方式
		  > 5. align-items：决定弹性元素在侧轴上的对齐方式，默认高度为容器高度
