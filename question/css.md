...
50. 什么是包含块，对于包含块的理解?
	- A：包含块（containing block）就是元素用来计算和定位的一个框，它相当于一个相对的参考系，某元素的包含块就是最高包裹当前元素的块级元素或者是离它最近的祖先元素(大多数是父元素)

		> 一些包含块的规则：
		>
		> - 初始包含块（即HTML元素的父级document，绝大多数场景可以认为是HTML）其尺寸等同于浏览器可视窗口的大小
		>
		> - 如果一个元素的position是relative或者static，则其包含块由其最近的块容器祖先盒的content box 边界形成
		>
		> - 如果position值是absolute,则其包含块为最近的position值不是static的祖先元素
		> 	
		> 	如果该祖先元素为行内元素，则有如下规则：
		> 	
		> 	- 可以通过给内联元素的前后各生成一个宽度为0的内联盒子（inline box），那么这两个内联盒子的padding box外面的包围盒就是内联元素的包含块
		> 	- 如果该内联元素被跨行分割了，那么其包含块是未定义的，则元素的包含块定义为初始包含块
51. 为什么不建议使用统配符初始化 css 样式
