...
46. 什么是尾调用，使用尾调用有什么好处？
	- A：尾调用指的是函数的最后一步调用另一个函数，我们代码执行是基于执行栈的，所以当我们在一个函数里调用另一个函数时，我们会保留当前的执行上下文，然后再新建另外一个执行上下文加入栈中。使用尾调用的话，因为已经是函数的最后一步，所以这个时候我们可以不必再保留当前的执行上下文，从而节省了内存，这就是尾调用优化。

		```js
		function f(x){
		  return g(x);
		}
		//只有不再用到外层函数的内部变量,才能进行尾调用优化
		function addOne(a){
		  var one = 1;
		  function inner(b){
...
48. 说说你对轮询的理解
	- A：轮询是指在一定的时间间隔内，定时向服务器发送请求，获取最新数据的过程，轮询通常用于从服务器获取实时更新的数据。

		> - 轮询与长轮询：轮询是每隔一段固定时间间隔向服务器发送请求。长轮询是先发送一个请求，服务器如果没有数据更新，则不会立即返回而是将请求挂起，直至有数据更新再返回结果
...
51. 什么是浏览器的同源政策？
	...
	- A：同源政策指的是未经许可的情况下，浏览器不允许一个域下的js脚本去访问（获取）另一个域的页面内容。同源指的是协议、域名、端口号必须相同，否则不属于同一个域。

		> 同源政策限制了三个方法：
		>
		> - 第一个是当前域下的js脚本不能够访问其他域下的cookie、localStorage 和 indexDB
		> - 第二是当前域下的js脚本不能操作或访问其他域下的DOM
		> - 第三是当前域下ajax无法发送跨域请求
		>
		> 同源政策的目的是为了保护用户的信息安全，它只是通过对网页中js脚本的一种限制，从而减少了恶意网站对用户的攻击风险。
52. Math.ceil()、Math.round()、Math.floor()三者的区别是什么？
	- A：

		- `Math.ceil()`：向上舍入，返回大于或等于给定数字的最小整数
		- `Math.round()`：四舍五入，返回最接近给定数字的整数，如果小数部分大于等于 0.5，则向上舍入，否则向下舍入
		- `Math.floor()`：向下舍入，返回小于或等于给定数字的最大整数

		```js
		const num1 = 7.2;
		const num2 = 7.7;
		
		const ceil = Math.ceil(num1); // 8
		const ceil = Math.ceil(num2); // 8
		
		const round = Math.round(num1); // 7
		const round = Math.round(num2); // 8
		
		const floor = Math.floor(num1); // 7
		const floor = Math.floor(num2); // 7
		```
53. get 和 post 请求在缓存方面的区别
	- A：缓存一般只适用于那些不会更新服务端数据的请求。

		> - 一般 get 请求都是查找请求（比如用户获取数据），不会对服务器资源数据造成修改，可以不用每次都与数据库连接，所以通常对get请求进行缓存
		> - post 请求一般都会对服务器数据造成修改，因此很少对post请求进行缓存

54. [] == ![]结果是什么？
	- A：结果为true
	- 解析：会先对!操作符进行运算，`!`运算符会先将[]转换为布尔值然后再取反，所有对象转换为布尔值都为true再取反为false。`==`运算符中存在布尔值时会先将布尔值转为数值 类型再进行下一步操作，因此![]最终为0，而对象（[]）与数值比较会回调用valueOf和toString方法，[]会转为空字符串，空字符串转为数字为0，所以最后0==0为ture。