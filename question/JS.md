...
17. 谈谈 Javascript 中的类型转换机制
	- A：JS中对各类运算符如果运算子的类型与预期不符合，就会触发类型转换机制，JS中常见的类型转换有：
		...
		- 隐式转换
			...
			> 隐式转换是JS自动对数据进行类型转换，不是我们通过方法来手动转换的。通常在我们进行算术运算和比较运算以及if、white中的条件表达式时会发生自动转换
			>
			> - 当进行加法运算时，一旦存在字符串，会自动转化为字符串。先将复合类型的值转为原始类型的值，再将原始类型的值转为字符串
			>
			> 	```js
			> 	'5' + 1 // '51'
			> 	'5' + true // "5true"
